...
class GPWrapper():
    ...
    def sort_cuda_devices(self):
        memory_map = dict()
        logging.getLogger(self.logger).warning(f"Torch Device Count: {torch.cuda.device_count()}")
        ...
        return {'cuda:' + str(k):v for k, v in memory_map.items()}
...
class ExactGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        ...
        self.mean_module = ConstantMean()
        #self.covar_module = GridKernel(ScaleKernel(RBFKernel()),grid=gpytorch.utils.grid.create_grid(grid_sizes=[15,15],grid_bounds=[(workspace.xmin,workspace.xmax),(workspace.ymin,workspace.ymax)]))
        self.covar_module = ScaleKernel(RBFKernel())
...
class TorchExactGp(GPWrapper):
    def __init__(self, X, Y, logger, verbose=True, use_mlp_mean=False,force_cpu=True,gpu_num=None):
        ...
        try:
            logging.getLogger(logger).warning(f"VIS: {os.environ['CUDA_VISIBLE_DEVICES']}")
        except KeyError:
            logging.getLogger(logger).warning("CUDA Visible devices not set")

        allocator = "mem"

        self.logger = logger
        if torch.cuda.is_available() and not force_cpu:
            #Make it so they wait to calculate the cuda usage until others have loaded
            if gpu_num is not None:
                ...
                if allocator == "mem":
                    ...
                    logging.getLogger(logger).warning(f"cuda devices: {self.sort_cuda_devices()}")
                    iterable = sorted([(memory, device) for device,memory in list(self.sort_cuda_devices().items())])
                elif allocator == "random":
                    num_devices = torch.cuda.device_count()
                    self.sort_cuda_devices()
                    iterable = [(0,f"cuda:{i}") for i in random.sample(range(1,num_devices+1),k=num_devices)]

                else:
                    raise Exception("Bad Allocator")

                for free,device in iterable:
                    try:
                        #torch.cuda.set_per_process_memory_fraction(1/8, device)
                        self.dev = device
                        self.likelihood = self.likelihood.cuda(device)
                        self.model = self.model.cuda(device)
                        self.using_cuda = True
                        found_device = True
                        logging.getLogger(logger).info(f"Device {device}, Free {free}")
                    except RuntimeError as e:
                        logging.getLogger(logger).warning(f"Couldn't use best device {device} due to {e}")
                if not found_device:
                    raise Exception("Couldn't fit into any cuda memory")

    def take_off_cuda(self):
        self.dev = None
    ...
    def fit(self, X, Y, input_uncertanties=None, early_stopping_delta=0.001,
        ...
        iterator = tqdm(range(optimization_steps), file=TqdmToLogger(logging.getLogger(self.logger), logging.INFO),
                        desc="Training Exact GP")
        scheduler = ReduceLROnPlateau(optimizer)
        best_loss = float("inf")
        best_params = None

        loss_plateau_iters = 0
        for i in iterator:
            ...
            if loss < best_loss:
                ...
                loss_plateau_iters+=1
            if loss_plateau_iters>=50:
                logging.getLogger(self.logger).debug(f"Exiting on plateau: {best_loss}")
                break
    ...
    def predict(self, X_t: np.ndarray, return_std=False, full_cov=False):
        with torch.no_grad(), gpytorch.settings.fast_computations(False,True,True), gpytorch.settings.fast_pred_var(), gpytorch.settings.max_root_decomposition_size(10):
            #logging.getLogger(self.logger).info(X_t)
            ...
            if return_std:
                ...
                raise Exception()
    def covar(self,X_t):
        with torch.no_grad(), gpytorch.settings.fast_computations(), gpytorch.settings.fast_pred_var(), gpytorch.settings.max_root_decomposition_size(
                10):
            # logging.getLogger(self.logger).info(X_t)
            pytorch_input = torch.from_numpy(X_t).float()
            if self.using_cuda:
            ...
            pred = self.likelihood(self.model(pytorch_input))
            # pred = self.model(pytorch_input)
            covar =pred.covariance_matrix.cpu().detach().numpy()
            return covar
...
class TorchSparseUncertainGPModel(GPWrapper):
    ...
    def load(fname):
        ...
        state_dict = torch.load(fname)
        # TODO does the likeliehood have any parameters? probably not right
