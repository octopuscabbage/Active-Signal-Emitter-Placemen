...
class CriticalPathVisualizer(Visualizer):
    ...
    def __init__(self, auv, pomcp_planner: POMCPPlanner, data_model: DataModel, workspace: Workspace, vbounds=None, screen_size=(20, 10), fignum=2):
        ...
        if workspace.dimensions() == 3:
            self.visualizer_ax = self.fig.add_subplot(111, projection="3d")
        else:
            self.visualizer_ax = self.fig.add_subplot(111)
    ...
    def update(self):
        ...
        if self.model.Xs.size:
            self.visualizer_ax.scatter(self.model.Xs[:, 0], self.model.Xs[:, 1], self.model.Xs[:, 2],
                                       s=20, marker="o", c="b", vmin=self.vmin, vmax=self.vmax)
        ...
        if path is not None and path.size:
            ...
            self.visualizer_ax.scatter(path[:, 0], path[:, 1], path[:, 2], s=20,
                                       marker="x", c='r', vmin=self.vmin, vmax=self.vmax)
