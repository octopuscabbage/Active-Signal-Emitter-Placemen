...
from sample_sim.motion_models.DubinsAirplane.ElementaryFunctions import max, min
...
def drawline(w1=None, q1=None, w2=None, q2=None, step=None):
    ...
    r.shape = (3,1)    
    while np.all(np.dot( (last_col - w2).T,q2 ) <= 0):
        s = s + step
        w1.shape = (3,1)
        q1.shape = (3,1)
        new_col = w1+s*q1
...
def drawspiral(R=None, gam=None, c=None, psi=None, lam=None, k=None, w=None, q=None, step=None):
    ...
    while ( required_crossing > 0 ) or ( (halfplane <= 0).all() ):
        ...
        if len(NrNc)==1 :
            ...
            NrNc_ind = NrNc[1]
            if np.all(np.sign(halfplane) != np.sign( np.dot( (r[0:2,NrNc_ind-1]-w[0:2].T ), q[0:2]) )):
                halfplane = np.dot( (r[0:2,NrNc_ind-1] - w[0:2] ).T, q[0:2] )
                required_crossing = required_crossing - 1
    return r

def ExtractDubinsAirplanePath(DubinsAirplaneSolutions=None, step=0.001):
